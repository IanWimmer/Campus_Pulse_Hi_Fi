...
export const Carousel = ({
  ...
  /*
  useEffect(() => {
    if (!containerRef.current || !cardRef.current || viewportWidth === 0)
      return;
    ...
    console.log(
      `Actual Card vs Viewport: ${calculatedPercent.toFixed(
        2
      )}% (Target: ${CARD_WIDTH_UNIT}%)`
    );
    console.groupEnd();
  }, [viewportWidth, index]);
  */
  ...
  const onCardClick = (item: EventType) => {
  ...
  const { clicked: clickedPrev, handleClick: handleClickPrev } =
    useClickAnimationImmediate(handlePrev);
  const { clicked: clickedNext, handleClick: handleClickNext } =
    useClickAnimationImmediate(handleNext);
  ...
  const xOffsetPx =
    viewportWidth / 2 - cardWidthPx / 2 - index * (cardWidthPx + GAP_PIXELS);
  ...
  const renderItem = (item: EventType) => {
    const cardProps = {
      ...
      tall: true,
      height: "h-full",
      enrolled: item.user_enrolled,
      onCancel: () => onEventCancel(item.id, true),
    };
  ...
  if (!items || items.length === 0) {
    return (
      <div
        className={clsx(
          "relative w-screen flex items-center justify-center",
          height
        )}
      >
        No events found
      ...
      >
        {items.map((item, i) => (
          <motion.div
            ...
            className={clsx("shrink-0 h-full transition-opacity duration-300")}
